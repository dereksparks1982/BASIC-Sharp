# BSIR Meaning Contract v0.1.32

## Identity

BSIR remains `bsir.debug.json`. `version` is `0.1.32`. The `meaning_profile` field is `bsharp.meaning.v1` for Profile 1 documents or `bsharp.meaning.v2` when creator text is present.
...
- Profile 1 normalized meaning and fingerprints may not change merely because v0.1.32 is installed.
