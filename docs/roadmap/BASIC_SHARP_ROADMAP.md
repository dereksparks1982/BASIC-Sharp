...
-> Small compiler subset error contract under Ruby referee  [CURRENT CANDIDATE: v0.1.57]
...
2. Commit and tag v0.1.57 immediately after every gate passes, then capture the accepted full-project snapshot.
...
## v0.1.57 IR emitter lane guardrail
...
## v0.1.57 error contract lane guardrail

The v0.1.57 lane is governed by `spec/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_ERROR_CONTRACT_v1.json`, `docs/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_ERROR_CONTRACT_v0_1_52.md`, and `compiler/small_compiler_subset_error_contract.rb`. It locks stable, plain-English error IDs and messages for invalid small compiler subset examples under the Ruby referee. It must not become the production compiler path and must not add Profile 8, new syntax, runtime changes, BSharp Bytecode changes, web export, browser work, engine bridge, or Ruby retirement.
...
## v0.1.57 scene/block expansion lane guardrail

The v0.1.57 lane is governed by `spec/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_SCENE_BLOCK_EXPANSION_v1.json`, `docs/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_SCENE_BLOCK_EXPANSION_v0_1_53.md`, and `compiler/small_compiler_subset_scene_block_expansion.rb`. It expands the sealed subset to larger ordered scene/block fixtures under Ruby Parser plus SemanticResolver referee.
...
After v0.1.57 is accepted, the next likely self-hosting bridge build is v0.1.57 Subset Symbol Table Contract, unless validation shows a narrower parser/IR repair is needed first.
...
### v0.1.57 small compiler subset BSBC emission

BASIC# v0.1.57 adds the first small compiler subset lane that emits real BSBC bytecode under Ruby referee control. It proves source -> BSharp IR -> BSBC bytes -> bytecode loader for sealed fixtures without replacing Ruby and without claiming BASIC# is self-hosted.
...
### v0.1.57 small compiler subset BSBC golden parity

BASIC# v0.1.57 adds the BSBC Golden Parity Harness under Ruby referee control. It locks approved subset source -> BSharp IR -> BSBC bytes -> bytecode loader summaries against sealed golden expectations without replacing Ruby and without claiming BASIC# is self-hosted.
