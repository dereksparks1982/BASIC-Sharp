...
v0.1.57 adds a plain-English error contract for invalid small compiler subset examples while Ruby remains the referee.
...
- v0.1.57 build handshake, validation, changelog, session log, runtime contract, and changed-files record
...
The first v0.1.57 candidate failed at `tools/text_value_stress.rb` because the versioned text-value Save fixture hash was stale. This repaired package carries the corrected fixture hash and adds the requested Five Point Paradigm plus Documentation Map cleanup.
