...
- **BSharp Bytecode** and **BSBC** are the protected names for the compact execution artifact governed by `bsharp.bytecode.v1`. The accepted Ruby bootstrap may emit deterministic `.bsbc` files, load and completely validate them into a deeply frozen trusted model, reconstruct `.bsbc.txt` diagnostic disassembly, and execute Profile 1 through the **BSharp Virtual Machine**. The first VM interprets the validated bytecode model directly and must not reconstruct BSIR or call the reference Ruby runtime. It remains Ruby-hosted bootstrap machinery. BSharp Save and BSharp ASK integration through the VM require later approved builds.
