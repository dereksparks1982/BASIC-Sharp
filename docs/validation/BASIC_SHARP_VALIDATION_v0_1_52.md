...
The first v0.1.61 candidate failed at `tools/text_value_stress.rb` with `Save fixture hash: FAIL` and restored exact v0.1.51. The repaired candidate updates the v0.1.61 text-value Save fixture hash and must pass `tools/text_value_stress.rb` before acceptance.
