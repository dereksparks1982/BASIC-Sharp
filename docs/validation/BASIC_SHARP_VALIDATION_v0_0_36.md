...
The installer must verify clean `main` at commit `8c5f096`, tag `v0.0.35`, validate every base and payload hash, install exactly the manifest scope, run the complete native suite and every focused/stress lane, and restore exact v0.0.35 after any post-mutation failure.
...
- Profile 2 text-value stress Save hash refreshed for the v0.0.36 creator-version identity; rerun required and retained in the final sweep.
