...
v0.0.13 is an incremental changed-files-only patch over the exact v0.0.12 baseline.

## Rejected archive result

A previous v0.0.13 archive was installed and tested by Derek. The code passed:
...
The archive was rejected because it also contained unchanged v0.0.12 files. That result proves the code path but does not accept the package.
...
```text
DKScript Ruby Bootstrap Compiler v0.0.13
statements: 7
kinds: 1
definitions: 5
facts: 4
...
```text
BASIC# Runtime Stress Test v0.0.13
Things: 504
...
The final corrected payload was generated from the actual difference between a clean v0.0.12 tree and the v0.0.13 candidate.
...
- no unchanged v0.0.12 file in the payload;
...
- applying the corrected payload to clean v0.0.12 reproduces the intended v0.0.13 tree;
...
Pending. Derek must restore the local repository to `v0.0.12`, apply the corrected archive, and rerun the full validation before v0.0.13 is committed or tagged.
