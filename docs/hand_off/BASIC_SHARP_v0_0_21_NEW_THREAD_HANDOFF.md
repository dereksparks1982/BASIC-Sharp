# BASIC# v0.0.21 New Thread Handoff
...
Owner installs and reviews the candidate. If accepted, commit and tag v0.0.21. Then read the current Company Bible and continuation records before proposing the next version. The roadmap currently points to Save/Load World State.

## Corrected-package requirement

Use only the corrected v0.0.21 ZIP. The first ZIP rejected before mutation because its `README.md` base hash did not match accepted commit `c94faec`. The corrected package was rebuilt from the exact accepted v0.0.20 file state and reran the full clean-base proof.
