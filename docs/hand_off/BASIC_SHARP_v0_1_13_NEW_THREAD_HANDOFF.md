# BASIC# v0.1.13 New Thread Handoff
...
```text
Version: v0.1.12 Plain-Language Runtime Trace
Commit: 25c9265
Tag: v0.1.12
...
The previously installed v0.1.13 ZIP is rejected. Its code passed all tests, but the archive incorrectly included unchanged v0.1.12 files while claiming to be an incremental changed-files-only package.
...
unzip -o "/home/dereksparks1982/Downloads/DKScript_Ruby_Bootstrap_Compiler_v0_1_13_FOCUSED_RUNTIME_STRESS_TEST_AND_CONTRACT_HARDENING_CORRECTED_CHANGED_FILES_ONLY.zip" -d "/home/dereksparks1982/DKLab/Projects/DKScript"
```

The hard reset restores all tracked files to the accepted v0.1.12 baseline. The corrected ZIP then overwrites the rejected v0.1.13 candidate files with the verified incremental payload.
...
```bash
git add . && \
git commit -m "DKScript v0.1.13 Focused Runtime Stress Test and Contract Hardening" && \
git tag -a v0.1.13 -m "DKScript v0.1.13 Focused Runtime Stress Test and Contract Hardening" && \
git status
```

## Continuation point

After v0.1.13 is accepted and tagged, prepare a tightly scoped v0.1.14 proposal. Kind Families are the current likely direction, but no v0.1.14 implementation begins without Derek's explicit approval.
