...
Before Git acceptance, snapshot review caught stale canonical README current-build lines left from Slice 2. The v0.0.76 repair corrects those lines and hardens the README Current Release Truth Gate so the exact canonical current milestone/build lines are checked across the full README rather than only the introductory release block.
