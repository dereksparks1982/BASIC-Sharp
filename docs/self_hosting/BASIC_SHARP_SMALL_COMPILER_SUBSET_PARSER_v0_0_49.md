# BASIC# Small Compiler Subset Parser v0.0.49
...
v0.0.49 adds the first small compiler subset parser implementation for the self-hosting runway. It parses deterministic subset records from `compiler/tokenizer_reader.rb` reader records, then compares those records against the existing Ruby Parser referee.
...
`compiler/small_compiler_subset_parser.rb` is not the production parser authority in v0.0.49. Normal compilation still routes through `compiler/parser.rb`.
...
5. Derek commits the exact accepted tree and tags `v0.0.49`.
...
The parser lane is the required input for the next self-hosting bridge record: `spec/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_IR_EMITTER_v1.json`. The v0.0.63 emitter may consume parser records, but the parser remains non-production and Ruby remains the referee.
