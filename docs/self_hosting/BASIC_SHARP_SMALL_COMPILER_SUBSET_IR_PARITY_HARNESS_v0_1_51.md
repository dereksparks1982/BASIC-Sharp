# BASIC# Small Compiler Subset IR Golden Parity Harness v0.1.53
...
v0.1.53 adds the first golden BSharp IR parity harness for the small compiler subset. The harness computes deterministic SHA256 digests from normalized BSharp IR emitted by `compiler/small_compiler_subset_ir_emitter.rb` and compares those digests against the Ruby Parser plus SemanticResolver referee.
...
The IR golden parity lane is the required valid-program guard for `spec/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_ERROR_CONTRACT_v1.json`. The v0.1.53 error contract locks invalid-program diagnostics while this v0.1.51 parity harness continues to protect valid small-subset BSharp IR output.
