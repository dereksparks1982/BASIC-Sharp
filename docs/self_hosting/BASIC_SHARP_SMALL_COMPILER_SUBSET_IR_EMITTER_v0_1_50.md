# BASIC# Small Compiler Subset IR Emitter v0.1.63
...
v0.1.63 adds the first small compiler subset BSharp IR emitter. It takes output from `compiler/small_compiler_subset_parser.rb`, builds a controlled compiler-subset program, emits resolved BSharp IR, and compares that IR against the existing Ruby Parser plus SemanticResolver referee.
...
Ruby remains the production parser, production resolver, production compiler path, and referee in v0.1.63.
...
The IR emitter lane is the required input for the next self-hosting bridge record: `spec/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_IR_PARITY_HARNESS_v1.json`. The v0.1.63 parity harness may compute locked golden BSharp IR digests from emitter records, but the emitter remains non-production and Ruby remains the referee.
