# BASIC# Small Compiler Subset IR Emitter v0.1.50
...
v0.1.50 adds the first small compiler subset BSharp IR emitter. It takes output from `compiler/small_compiler_subset_parser.rb`, builds a controlled compiler-subset program, emits resolved BSharp IR, and compares that IR against the existing Ruby Parser plus SemanticResolver referee.
...
Ruby remains the production parser, production resolver, production compiler path, and referee in v0.1.50.
...
5. Derek commits the exact accepted tree and tags `v0.1.50`.

## Next runway step

After v0.1.50 is accepted, the next likely self-hosting build is the small compiler subset emits BSBC bytecode lane, still under Ruby referee control.
