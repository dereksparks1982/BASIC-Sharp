# BASIC# v0.0.83 Native Symbol Resolution Integration

BASIC# v0.0.83 is Self-Hosting Milestone 2, Slice 10. It places a BASIC#-authored symbol decision component inside the bounded independent compiler after native parser dispatch and native semantic routing.
...
The accepted v0.0.82 independent compiler produces generation #1 of the symbol artifact after only the target-version identity is advanced. v0.0.83 then recompiles the same BASIC# source through generation #1 and must produce byte-identical generation #2 BSBC and readable disassembly.
