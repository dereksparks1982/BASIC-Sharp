# BASIC# Small Compiler Subset Semantic Resolver v0.1.74
...
## v0.1.73 object interaction proof

The v0.1.74 semantic-resolver lane explicitly carries the accepted `(open`, `(close`, `(lock`, and `(take` actions through the independent resolver and requires exact parity with the Ruby referee. Those actions continue to canonicalize to the existing state-change and carry primitives.
