...
assert_contract!(spec.fetch('status') == 'native_symbol_resolution_integration_under_ruby_referee', 'wrong contract status')
...
all.each do |fixture|
  ...
  assert_contract!(record.fetch(:native_symbol_invocation_count).positive?, "native symbol resolver was not invoked for #{fixture.fetch('name')}")
