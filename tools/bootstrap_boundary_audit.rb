...
assert_contract!(record.fetch(:stage_count) >= 10, 'not enough boundary stages')
...
assert_contract!(record.fetch(:checks).fetch(:native_symbol_resolution_declared), 'native symbol resolution integration boundary missing')
...
puts 'v0.1.83 Slice 10 milestone gate remains guarded: PASS'
