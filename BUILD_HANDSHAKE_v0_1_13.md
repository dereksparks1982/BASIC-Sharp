# DKScript Build Handshake v0.1.13
...
The corrected v0.1.13 package is an incremental changed-files-only patch over the exact `v0.1.12` tag.
...
Earlier v0.1.13 archives are rejected. One was installed and passed all runtime tests but incorrectly included unchanged v0.1.12 files. It must not be committed, tagged, or used as a baseline.

The failure record is preserved at:

```text
docs/audit/BASIC_SHARP_v0_1_13_REJECTED_PACKAGE_AUDIT.md
```
...
- Preserve all accepted v0.1.12 plain-language trace behavior.
...
- Advance active version surfaces to 0.1.13.
...
```text
docs/changed_files/BASIC_SHARP_CHANGED_FILES_v0_1_13.txt
```
...
The user's current working tree contains the rejected, uncommitted v0.1.13 install. Restore tracked files to the accepted baseline before applying the corrected package:

```bash
git reset --hard v0.1.12
```

No accepted commit is removed. The rejected v0.1.13 candidate was never committed or tagged.
...
The final payload was generated from the actual file difference between a clean v0.1.12 tree and the v0.1.13 candidate.
...
- extracted payload reproduces the tested v0.1.13 tree;
...
Pending. The rejected package passed, but the corrected archive must be applied from `v0.1.12` and validated again before v0.1.13 is accepted.

## Current continuation point

1. Download the corrected package.
2. Reset the project to `v0.1.12`.
3. Apply the corrected package.
4. Run the complete validation command.
5. If it passes, commit and tag v0.1.13.
6. Resume tomorrow using `docs/hand_off/BASIC_SHARP_v0_1_13_NEW_THREAD_HANDOFF.md`.
7. Do not begin v0.1.14 without an exact proposal and Derek's explicit approval.

## Required input and artifacts for continuation

- accepted local Git tag `v0.1.12`;
- corrected v0.1.13 ZIP in `/home/dereksparks1982/Downloads/`;
