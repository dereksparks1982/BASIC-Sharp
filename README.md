# BASIC# Ruby Bootstrap Compiler v0.1.67

> A scripting language made for non-programmers, by non-programmers.

BASIC# v0.1.67 adds plain-English 2D and 3D movement intent while preserving BSBC execution and runtime behaviour proof while release gates check deterministic fixture hashes, sealed inventory byte counts, package payload hashes, and changed-file scope before acceptance testing.
...
v0.1.67 teaches movement declarations that W can mean screen-up in 2D contexts or forward in 3D contexts, with keyboard and controller inputs mapped underneath. The BSBC execution corpus remains sealed at 17 fixtures and 55 event executions as the carried-forward runtime proof floor.

v0.1.67 names 2D and 3D movement intent directly, emits `move_3d` for 3D world movement, and records that W can mean forward in 3D contexts.
...
Milestone: v0.1.67 Self-Hosting Milestone 1 under Ruby referee control
...
## v0.1.67 release hardening note

v0.1.67 records the v0.1.63 rejected-candidate lesson: where one stale deterministic hash exists, more may exist. The package must sweep the whole family and audit final payload bytes before acceptance testing.
