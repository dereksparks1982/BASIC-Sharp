# BASIC# Ruby Bootstrap Compiler v0.1.36

> A scripting language made for non-programmers, by non-programmers.

BASIC# v0.1.36 adds plain-English side-scroller movement while keeping polling, frame timing, velocity, gravity, jump gating, collision response, and engine calls beneath the creator-facing language:

```text
.bsharp source -> BSharp IR -> BSharp Bytecode -> validated BSharp VM
quoted text     -> exact UTF-8 value -> Save, ASK, restore, and parity
platform words  -> Profile 4 -> engine-neutral collision movement
```
...
docs/validation/BASIC_SHARP_VALIDATION_v0_1_36.md
```
...
Meaning profiles: bsharp.meaning.v1 through bsharp.meaning.v4
Bytecode profiles: bsharp.bytecode.v1 through bsharp.bytecode.v4
Version: 0.1.36
```
