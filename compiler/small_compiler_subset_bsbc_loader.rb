...
module BasicSharp
  class SmallCompilerSubsetBSBCLoaderError < ArgumentError; end

  class SmallCompilerSubsetBSBCLoader
    ...
    def self.read(path, expected_fingerprint: nil)
      ...
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode file not found: #{path}" unless File.file?(expanded)

      new(File.binread(expanded), source_label: path, expected_fingerprint: expected_fingerprint)
    rescue SystemCallError => error
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode could not be read: #{error.message}"
    end

    def initialize(bytes, source_label: '(memory)', expected_fingerprint: nil)
      unless bytes.is_a?(String)
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode input must be binary bytes.'
      end
    ...
    def verify_fingerprint!(expected)
      ...
      raise SmallCompilerSubsetBSBCLoaderError,
    ...
    def reject_implementation_leaks!
      ...
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode contains implementation-specific serialized data.'
    end

    def parse_header_and_directory!
      if @bytes.bytesize < BytecodeContract::HEADER_SIZE_BYTES
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode header is truncated.'
      end
      ...
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode magic bytes are wrong.' unless magic == BytecodeContract::MAGIC
      unless binary_version == BytecodeContract::BINARY_FORMAT_VERSION
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode binary format version #{binary_version} is not supported."
      end
      unless [1, 2, 3, 4, 5, 6, 7].include?(profile_version)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode profile format version #{profile_version} is not supported."
      end
      @profile_format_version = profile_version
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode header size is wrong.' unless header_size == BytecodeContract::HEADER_SIZE_BYTES
      expected_order = BytecodeContract.section_order_for_format(profile_version)
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode section count is wrong.' unless section_count == expected_order.length
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode section directory offset is wrong.' unless directory_offset == BytecodeContract::HEADER_SIZE_BYTES
      unless directory_entry_size == BytecodeContract::DIRECTORY_ENTRY_SIZE_BYTES
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode section-directory entry size is wrong.'
      end
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode reserved header field must be zero.' unless reserved_flags.zero?
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode file-size field does not match the file.' unless declared_file_size == @bytes.bytesize
      ...
      if directory_end > @bytes.bytesize
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode section directory is truncated.'
      end
      ...
      if ids.uniq.length != ids.length
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode contains a duplicate section.'
      end
      unless (expected_order - ids).empty? && (ids - expected_order).empty?
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode is missing a required section.'
      end
      unless ids == expected_order
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode sections are in the wrong order.'
      end
      ...
      @directory.each do |entry|
        ...
        if offset < minimum_data_offset || offset > @bytes.bytesize
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode section #{entry[:id]} has an out-of-range offset."
        end
        unless (offset % BytecodeContract::ALIGNMENT_BYTES).zero?
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode section #{entry[:id]} is not four-byte aligned."
        end
        section_end = checked_add(offset, length, "section #{entry[:id]}")
        if section_end > @bytes.bytesize
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode section #{entry[:id]} has an out-of-range length."
        end
        if offset < previous_end
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode sections overlap.'
        end

        expected_offset = align(previous_end)
        unless offset == expected_offset
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode contains an unexplained section gap.'
        end
        padding = @bytes.byteslice(previous_end, offset - previous_end)
        unless padding.nil? || padding.bytes.all?(&:zero?)
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode alignment padding must be zero-filled.'
        end
      ...
      unless previous_end == @bytes.bytesize
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode contains trailing unexplained data.'
      end
    end
    ...
    def parse_strings!
      ...
      entry.fetch(:count).times do
        ...
        unless value.valid_encoding?
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode string table contains invalid UTF-8.'
        end
        ...
        unless padding.bytes.all?(&:zero?)
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode string padding must be zero-filled.'
        end
      ...
      unless cursor == data.bytesize
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode STRS section contains a truncated record or trailing bytes.'
      end
      unless @strings.uniq.length == @strings.length
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode string table contains a duplicate deterministic entry.'
      end
      ...
      unless @strings.first(3) == mandatory
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode string table has the wrong mandatory identity prefix.'
      end
    end
    ...
    def parse_meta!
      ...
      data = section('META')
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode META record count must be one.' unless entry.fetch(:count) == 1
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode META section has the wrong length.' unless data.bytesize == META_SIZE_BYTES
      ...
      unless @strings.fetch(profile_index) == expected_profile
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode uses an unsupported bytecode profile.'
      end
      unless @strings.fetch(meaning_index) == expected_meaning
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode meaning profile does not match #{expected_meaning}."
      end
      unless @strings.fetch(fingerprint_algorithm_index) == expected_fingerprint
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode meaning-fingerprint algorithm is not supported.'
      end
    ...
    def parse_kinds!
      ...
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode KIND section length does not match its record count.' unless data.bytesize == expected
      ...
      @kinds = entry.fetch(:count).times.map do |index|
        ...
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode contains duplicate Kind '#{name}'." if seen_names[name]
        seen_names[name] = true
        unless parent_index == BytecodeContract::NO_REFERENCE_U32 || parent_index < entry.fetch(:count)
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode Kind '#{name}' has an invalid parent index."
        end
        unless source_order == index
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode Kind source order is not canonical.'
        end
      ...
      @kinds.each_with_index do |entry, index|
        ...
        if parent >= index
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode Kind '#{entry.fetch(:name)}' must appear after its parent."
        end
      end
    end
    ...
    def parse_things!
      ...
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode THNG section length does not match its record count.' unless data.bytesize == expected
      ...
      @things = entry.fetch(:count).times.map do |index|
        ...
        name = @strings.fetch(name_index)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode contains duplicate Thing '#{name}'." if seen_names[name]
        seen_names[name] = true
        unless definition_order == index
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode Thing definition order is not canonical.'
        end
    ...
    def parse_start_records!
      ...
      unless cursor == data.bytesize
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode STRT section contains a truncated record or trailing bytes.'
      end
    end
    ...
    def parse_events!
      ...
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode EVNT section length does not match its record count.' unless data.bytesize == expected
      ...
      @events = entry.fetch(:count).times.map do |index|
        ...
        unless block_index == index
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode WHEN block order is not canonical.'
        end
        unless source_order == index
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode WHEN source order is not canonical.'
        end
        {
    ...
    def parse_if_rules!
      ...
      @if_rules = entry.fetch(:count).times.map do |index|
        ...
        if %w[ALL_CONDITIONS ANY_CONDITIONS].include?(condition.fetch(:name))
          ...
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode compound IF must contain at least two clauses.' unless clause_count >= 2
          clauses = clause_count.times.map do
            ...
            if %w[ALL_CONDITIONS ANY_CONDITIONS].include?(clause.fetch(:name))
              raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode compound IF clauses cannot contain nested condition groups.'
            end
        ...
        unless block_index == event_count + index
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode IF block order is not canonical.'
        end
        unless source_order == index
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode IF source order is not canonical.'
        end
        if otherwise_block_index != BytecodeContract::NO_REFERENCE_U32
          ...
          unless otherwise_block_index == next_otherwise_block
            raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode OTHERWISE block order is not canonical.'
          end
      ...
      unless cursor == data.bytesize
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode IFRL section contains a truncated record or trailing bytes.'
      end
    end
    ...
    def parse_game_section!(id, label)
      ...
      unless data.dup.force_encoding(Encoding::UTF_8).valid_encoding?
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{id} section is not valid UTF-8."
      end
      parsed = JSON.parse(data)
      unless parsed.is_a?(Array) && parsed.length == directory_entry(id).fetch(:count)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{label} count does not match its section directory."
      end
      deep_symbolize(parsed)
    rescue JSON::ParserError
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{id} section is not valid canonical game data."
    end
    ...
    def parse_code!
      ...
      unless block_count == entry.fetch(:count)
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode CODE block count disagrees with its directory entry.'
      end
      ...
      if directory_end > data.bytesize
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode CODE block directory is truncated.'
      end
      ...
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode contains a duplicate block identifier.' unless ids.uniq.length == ids.length
      unless ids == (0...block_count).to_a
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode block identifiers must be zero-based and ordered.'
      end
      ...
      @blocks = rows.map do |row|
        ...
        unless (offset % BytecodeContract::ALIGNMENT_BYTES).zero?
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode code block is not four-byte aligned.'
        end
        block_end = checked_add(offset, length, 'CODE block')
        if offset < previous_end
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode code blocks overlap.'
        end
        unless offset == previous_end
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode CODE section contains an unexplained block gap.'
        end
        if block_end > data.bytesize
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode code block extends outside the CODE section.'
        end
        ...
        unless cursor == block_end
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode instruction count or block length is inconsistent.'
        end
        previous_end = block_end
        { id: row.fetch(:id), instructions: instructions }
      end
      unless previous_end == data.bytesize
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode CODE section contains trailing unexplained data.'
      end
    end
    ...
    def parse_instruction_record(data, cursor, boundary, context:)
      ...
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode instruction reserved field must be zero.' unless reserved.zero?
      name = instruction_name!(opcode)
      expected = INSTRUCTION_OPERAND_COUNTS.fetch(name)
      unless operand_count == expected
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode instruction #{name} has the wrong operand count."
      end
      ...
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode instruction 0x#{format('%02X', opcode)} is not defined."
    end

    def parse_condition_record(data, cursor, boundary)
      ensure_record_boundary!(data, cursor, boundary, 4, 'condition header')
      opcode, operand_count, reserved = data.byteslice(cursor, 4).unpack('CCv')
      raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode condition reserved field must be zero.' unless reserved.zero?
      name = condition_name!(opcode)
      expected = CONDITION_OPERAND_COUNTS.fetch(name)
      unless operand_count == expected
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode condition #{name} has the wrong operand count."
      end
    ...
    def validate_start_instruction!(record)
      ...
      unless START_INSTRUCTIONS.include?(name)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode instruction #{name} is not allowed in STRT."
      end
      ...
      when 'START_TEXT_VALUE'
        ...
        if @strings.fetch(operands[1]) == 'damage'
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode damage cannot use START_TEXT_VALUE.'
        end
      end
    end
    ...
    def validate_action_instruction!(record)
      ...
      unless ACTION_INSTRUCTIONS.include?(name)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode instruction #{name} is not allowed in CODE."
      end
      operands = record.fetch(:operands)
      case name
      when 'DAMAGE'
    ...
    def validate_condition_operands!(name, operands)
      ...
      when 'ALL_CONDITIONS', 'ANY_CONDITIONS'
        ...
        unless operands[0].is_a?(Integer) && operands[0] >= 2
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{name} must contain at least two clauses."
        end
      end
    end
    ...
    def validate_count_agreement!
      ...
      expected.each do |id, count|
        unless directory_entry(id).fetch(:count) == count
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode META count disagrees with #{id}."
        end
      end
      ...
      unless @blocks.length == @events.length + @if_rules.length + otherwise_count
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode code-block count must equal WHEN, IF, and OTHERWISE blocks.'
      end
    end
    ...
    def validate_string_usage_and_order!
      ...
      unless encountered == (0...@strings.length).to_a
        if encountered.sort != (0...@strings.length).to_a
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode string table contains an unused entry.'
        end
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode string table order is not canonical.'
      end
    ...
    def validate_kind_ancestry!
      @kinds.each_index do |start|
        ...
        loop do
          ...
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode Kind ancestry contains a circle.' if seen[parent]
          seen[parent] = true
          cursor = parent
        end
      end
    ...
    def validate_selector_context!(selector, reference, allowed, label)
      unless allowed.include?(selector)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode selector #{selector} is not allowed for #{label}."
      end
      case selector
      when 'EXACT_THING'
      ...
      when 'NO_REFERENCE'
        unless reference == BytecodeContract::NO_REFERENCE_U32
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode NO_REFERENCE for #{label} must use 0xFFFFFFFF."
        end
      end
    end
    ...
    def selector_name!(code)
      ...
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode selector code 0x#{format('%02X', code & 0xFF)} is invalid." unless name
      name
    end
    ...
    def instruction_name!(opcode)
      ...
      if conditions.value?(opcode)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode instruction code 0x#{format('%02X', opcode)} is unknown."
      end
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode instruction code 0x#{format('%02X', opcode)} is reserved."
    end
    ...
    def condition_name!(opcode)
      ...
      if instructions.value?(opcode)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode condition code 0x#{format('%02X', opcode)} is unknown."
      end
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode condition code 0x#{format('%02X', opcode)} is reserved."
    end

    def validate_string_index!(index)
      unless index.is_a?(Integer) && index.between?(0, @strings.length - 1)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode string index #{index} is invalid."
      end
    end
    ...
    def validate_kind_index!(index)
      ...
      unless index.is_a?(Integer) && index.between?(0, count - 1)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode Kind index #{index} is invalid."
      end
    end
    ...
    def validate_thing_index!(index)
      ...
      unless index.is_a?(Integer) && index.between?(0, count - 1)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode Thing index #{index} is invalid."
      end
    end
    ...
    def validate_record_block_reference!(index)
      ...
      unless index.is_a?(Integer) && index.between?(0, count - 1)
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode code-block reference #{index} is invalid."
      end
    end
    ...
    def normalize_fingerprint(value)
      ...
      unless text.match?(/\A[0-9a-f]{64}\z/)
        raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode comparison fingerprint must be 64 lowercase hexadecimal characters.'
      end
      text
    end
    ...
    def require_profile_2!(name)
      ...
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{name} requires Profile 2 or later."
    end

    def require_profile_5!(name)
      return if [5, 6, 7].include?(@profile_format_version)

      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{name} requires Profile 5."
    end

    def require_profile_6!(name)
      return if [6, 7].include?(@profile_format_version)
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{name} requires Profile 6."
    end
    ...
    def validate_string_roles!
      @strings.each_with_index do |value, index|
        ...
        if roles.include?(:identifier) && value != normalize_string(value)
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode identifier string contains noncanonical semantic text.'
        end
        next unless roles.include?(:literal)

        if value.include?("\n") || value.include?("\r")
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode creator text must stay on one line.'
        end
        if value.include?('\\')
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode creator text cannot contain unsupported escape sequences.'
        end
        if value.include?('#{')
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode creator text cannot contain unsupported interpolation.'
        end
        if value.include?('"')
          raise SmallCompilerSubsetBSBCLoaderError, 'BSharp Bytecode creator text cannot contain an unescaped straight double quote.'
        end
      end
    end
    ...
    def validate_value_type_contracts!
      ...
      @start_records.each do |record|
        ...
        if assigned[key]
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode assigns starting value #{value_name} more than once for #{thing_display(thing_index)}."
        end
        assigned[key] = true
        established = schemas[value_name]
        if established && established != type
          raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode starting value #{value_name} changes its established value type."
        end
    ...
    def directory_entry(id)
      @directory.find { |entry| entry.fetch(:id) == id } || raise(SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode is missing section #{id}.")
    end

    def ensure_bytes!(data, cursor, length, label)
      end_offset = checked_add(cursor, length, label)
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{label} is truncated." if cursor.negative? || end_offset > data.bytesize
    end
    ...
    def ensure_record_boundary!(data, cursor, boundary, length, label)
      ...
      if cursor + length > boundary
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{label} crosses its block boundary."
      end
    end

    def checked_add(left, right, label)
      unless left.is_a?(Integer) && right.is_a?(Integer) && left >= 0 && right >= 0
        raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{label} geometry is invalid."
      end
      result = left + right
      raise SmallCompilerSubsetBSBCLoaderError, "BSharp Bytecode #{label} arithmetic overflowed." if result > 0xFFFF_FFFF
      result
    end
