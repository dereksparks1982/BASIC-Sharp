...
module BasicSharp
  class SmallCompilerSubsetBSBCEncoderError < ArgumentError; end

  class SmallCompilerSubsetBSBCEncoder
    # Independent Subset 0 BSBC encoder. Do not delegate to BytecodeEmitter.
    ...
    def write(path)
      ...
      unless destination.end_with?(BytecodeContract::EXTENSION)
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode output must end with #{BytecodeContract::EXTENSION}."
      end
    ...
    def validate_document!
      unless @document.is_a?(Hash) && @document['format'].to_s == 'bsir.debug.json'
        raise SmallCompilerSubsetBSBCEncoderError, 'BSharp Bytecode emission requires resolved BSharp IR.'
      end
      ...
      unless errors.empty?
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode was not written because the program has #{errors.length} error#{errors.length == 1 ? '' : 's'}."
      end
      unless warnings.empty?
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode was not written because the program has #{warnings.length} warning#{warnings.length == 1 ? '' : 's'}."
      end
      ...
      unless profile.nil? || [BytecodeContract::MEANING_PROFILE, BytecodeContract::MEANING_PROFILE_2, BytecodeContract::MEANING_PROFILE_3, BytecodeContract::MEANING_PROFILE_4, BytecodeContract::MEANING_PROFILE_5, BytecodeContract::MEANING_PROFILE_6, BytecodeContract::MEANING_PROFILE_7].include?(profile)
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode does not support meaning profile '#{profile}'."
      end
      ...
      if text_used && ![BytecodeContract::MEANING_PROFILE_2, BytecodeContract::MEANING_PROFILE_3, BytecodeContract::MEANING_PROFILE_4, BytecodeContract::MEANING_PROFILE_5, BytecodeContract::MEANING_PROFILE_6, BytecodeContract::MEANING_PROFILE_7].include?(profile)
        raise SmallCompilerSubsetBSBCEncoderError, 'Creator-facing text values require bsharp.meaning.v2 or later in BSharp IR.'
      end
      if profile == BytecodeContract::MEANING_PROFILE_2 && !text_used
        raise SmallCompilerSubsetBSBCEncoderError, 'bsharp.meaning.v2 requires at least one creator-facing text value.'
      end
      ...
      if profile == BytecodeContract::MEANING_PROFILE_3 && !game_used
        raise SmallCompilerSubsetBSBCEncoderError, 'bsharp.meaning.v3 requires game input or interaction meaning.'
      end
      ...
      if profile == BytecodeContract::MEANING_PROFILE_4 && !movement_used
        raise SmallCompilerSubsetBSBCEncoderError, 'bsharp.meaning.v4 requires movement meaning.'
      end
      number_features = document_uses_number_changes_or_comparisons?
      if profile == BytecodeContract::MEANING_PROFILE_5 && !number_features
        raise SmallCompilerSubsetBSBCEncoderError, 'bsharp.meaning.v5 requires number-change or threshold-comparison meaning.'
      end
      if profile == BytecodeContract::MEANING_PROFILE_6 && !document_uses_compound_if_conditions?
        raise SmallCompilerSubsetBSBCEncoderError, 'bsharp.meaning.v6 requires a compound IF condition.'
      end
      if profile == BytecodeContract::MEANING_PROFILE_7 && !document_uses_otherwise_branches?
        raise SmallCompilerSubsetBSBCEncoderError, 'bsharp.meaning.v7 requires an OTHERWISE branch.'
      end
    end
    ...
    def add_kind_with_ancestors(name, visiting = {})
      ...
      raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode cannot encode a Kind ancestry circle involving '#{normalized}'." if visiting[normalized]
    ...
    def build_things(_kind_rows)
      ...
      Array(@document['objects']).each_with_index.map do |entry, index|
        ...
        raise SmallCompilerSubsetBSBCEncoderError, "Duplicate Thing '#{name}' cannot be emitted." if @thing_indexes.key?(name)
        ...
        kind_name = normalize(entry['kind'])
        {
          name: name,
          name_string_index: intern(name),
          kind_index: @kind_indexes.fetch(kind_name) { raise SmallCompilerSubsetBSBCEncoderError, "Thing '#{name}' uses missing Kind '#{kind_name}'." },
    ...
    def lower_start(fact)
      ...
      when 'has'
        ...
        if fact.key?('text_value')
          raise SmallCompilerSubsetBSBCEncoderError, 'damage is a whole-number value and cannot store text.' if value_name == 'damage'
      ...
      raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode cannot lower a START fact: #{error.message}"
    end
    ...
    def lower_condition(condition)
      if condition.key?('connector')
        ...
        name = { 'and' => 'ALL_CONDITIONS', 'or' => 'ANY_CONDITIONS' }.fetch(connector) do
          raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode cannot lower compound connector '#{connector}'."
        end
      ...
      when 'has'
        ...
        if condition.key?('text_value')
          ...
          }.fetch(comparison) { raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode cannot lower comparison '#{comparison}'." }
      ...
      raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode cannot lower an IF condition: #{error.message}"
    end
    ...
    def lower_action(action)
      ...
      when 'change'
        ...
        if action.key?('value_name')
          ...
          raise SmallCompilerSubsetBSBCEncoderError, 'BSharp Bytecode cannot lower a state change without a state.' if state.empty?
      ...
      when 'cause'
        ...
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode Profile 1 does not support action '#{name}'."
      end
    rescue KeyError => error
      raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode cannot lower action '#{name}': #{error.message}"
    end

    def lower_selector(reference, context, optional: false)
      if reference.nil?
        return ['NO_REFERENCE', BytecodeContract::NO_REFERENCE_U32] if optional
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode requires a reference in #{context}."
      end
      ...
      when 'kind_set'
        ...
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode cannot encode reference type '#{type}' in #{context}."
      end
    end

    def kind_index(reference)
      name = normalize(reference['kind_name'] || reference['object_kind'])
      @kind_indexes.fetch(name) { raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode reference uses missing Kind '#{name}'." }
    end

    def exact_thing_index(reference)
      name = normalize(reference['name'] || reference['text'])
      @thing_indexes.fetch(name) { raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode reference uses missing Thing '#{name}'." }
    end

    def instruction(name, operands, display)
      expected = instruction_operand_count(name)
      raise SmallCompilerSubsetBSBCEncoderError, "#{name} requires #{expected} operands, got #{operands.length}." unless operands.length == expected
    ...
    def build_binary
      ...
      raise SmallCompilerSubsetBSBCEncoderError, 'BSharp Bytecode file-size calculation failed.' unless result.bytesize == file_size
      if @profile == BytecodeContract::PROFILE && FORBIDDEN_BINARY_TERMS.any? { |term| result.include?(term) }
        raise SmallCompilerSubsetBSBCEncoderError, 'BSharp Bytecode contains Ruby-specific serialized data.'
      end
    ...
    def encode_meta
      ...
      raise SmallCompilerSubsetBSBCEncoderError, 'Meaning fingerprint must be exactly 32 bytes.' unless fingerprint_bytes.bytesize == 32
    ...
    def u32(value)
      unless value.is_a?(Integer) && value.between?(0, 0xFFFF_FFFF)
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode operand is outside u32: #{value.inspect}"
      end
      value
    end
    ...
    def whole_number(value, label, minimum: 0)
      unless value.is_a?(Integer) && value.between?(minimum, BytecodeContract::WHOLE_NUMBER_RANGE.end)
        raise SmallCompilerSubsetBSBCEncoderError, "#{label} is outside #{minimum} through #{BytecodeContract::WHOLE_NUMBER_RANGE.end}."
      end
      value
    end
    ...
    def intern(value, role: :identifier)
      ...
      raise SmallCompilerSubsetBSBCEncoderError, 'BSharp Bytecode strings must be valid UTF-8.'
    end
    ...
    def text_value(value, label)
      ...
      raise SmallCompilerSubsetBSBCEncoderError, "#{label} is invalid: #{error.message}"
    end
    ...
    def atomic_write_pair(binary_path, binary_content, text_path, text_content)
      ...
      begin
        ...
        raise SmallCompilerSubsetBSBCEncoderError, "BSharp Bytecode could not write output: #{error.message}"
      ensure
