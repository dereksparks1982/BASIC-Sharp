...
module BasicSharp
  class SmallCompilerSubsetBSBCVirtualMachineError < ArgumentError; end

  # Independently executes only completely validated Subset 0 BSharp Bytecode models.
  # It intentionally does not depend on the production BytecodeVirtualMachine or BasicSharp::Runtime.
  class SmallCompilerSubsetBSBCVirtualMachine
    ...
    def initialize(loader, world_save: nil)
      unless loader.is_a?(SmallCompilerSubsetBSBCLoader)
        raise SmallCompilerSubsetBSBCVirtualMachineError,
              'The independent subset BSharp VM accepts only a successfully validated SmallCompilerSubsetBSBCLoader.'
      end
    ...
    def execute_context_action(action, object_name)
      ...
      raise SmallCompilerSubsetBSBCVirtualMachineError, "Context action object '#{object_name}' is not defined." unless index
    ...
    def execute_semantic_context_action(action, target_index)
      ...
      when 'cause'
        ...
        raise SmallCompilerSubsetBSBCVirtualMachineError, "BSharp VM context action does not support (#{name}."
      end
    end
    ...
    def validate_profile!
      ...
      unless supported.include?(pair)
        raise SmallCompilerSubsetBSBCVirtualMachineError, 'The independent subset BSharp VM cannot execute this bytecode profile.'
      end
    end
    ...
    def execute_start_records
      @program.fetch(:start_records).each do |record|
        ...
        when 'START_TEXT_VALUE'
          ...
          raise SmallCompilerSubsetBSBCVirtualMachineError, "The independent subset BSharp VM cannot execute START instruction #{record.fetch(:name)}."
        end
      end
    end
    ...
    def execute_block(block_index, actor_index:, context:, selections: [])
      ...
      rescue SmallCompilerSubsetBSBCVirtualMachineError, KeyError => runtime_error
        steps << {
    ...
    def action_selection(instruction, context)
      ...
      when 'EVERY_KIND'
        ...
        raise SmallCompilerSubsetBSBCVirtualMachineError, "Action selector #{selector} cannot select a Thing."
      end
    end
    ...
    def execute_instruction(instruction, target_index:, actor_index:)
      ...
      when 'UNLOCK'
        ...
        raise SmallCompilerSubsetBSBCVirtualMachineError, "The independent subset BSharp VM cannot execute #{instruction.fetch(:name)}."
      end
    end
    ...
    def materialize_event_reference(selector, reference, context)
      ...
      when 'BOUND_THAT_KIND'
        index = context[reference]
        raise SmallCompilerSubsetBSBCVirtualMachineError, "BASIC# could not resolve 'that #{kind_name(reference)}' for this caused event." if index.nil?
        thing_name(index)
      when 'ONE_KIND' then "one #{kind_name(reference)}"
      else
        raise SmallCompilerSubsetBSBCVirtualMachineError, "Caused-event selector #{selector} is not executable."
      end
    end
