...
class TestSelfHostingContract < Minitest::Test
  ...
  def test_contract_is_foundation_only
    ...
    assert_equal 'self_hosting_milestone_2_slice_10_native_symbol_resolution_integration_under_ruby_referee', spec.fetch('compiler_subset_status')
  end
  ...
  def test_tokenizer_reader_contract_is_linked
    ...
    assert_equal 'spec/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_PIPELINE_INDEPENDENCE_v1.json', documents.fetch('small_compiler_subset_pipeline_independence_spec')
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_pipeline_independence_implementation')))
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_pipeline_file')))
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_pipeline_independence_tool')))
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_pipeline_independence_test')))
    assert_equal 'spec/self_hosting/BASIC_SHARP_SMALL_COMPILER_SUBSET_DRIVER_INDEPENDENCE_v1.json', documents.fetch('small_compiler_subset_driver_independence_spec')
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_driver_independence_implementation')))
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_driver_file')))
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_driver_independence_tool')))
    assert File.file?(File.join(ROOT, documents.fetch('small_compiler_subset_driver_independence_test')))
    ...
    assert_equal 'spec/self_hosting/BASIC_SHARP_NATIVE_SYMBOL_RESOLUTION_INTEGRATION_v1.json', documents.fetch('native_symbol_resolution_spec')
    assert_equal 'compiler/native/first_bsharp_symbol_resolver.bsharp', documents.fetch('native_symbol_resolution_source')
    assert_equal 'compiler/native/first_bsharp_symbol_resolver.bsbc', documents.fetch('native_symbol_resolution_artifact')
    %w[native_symbol_resolution_source native_symbol_resolution_artifact native_symbol_resolution_disassembly native_symbol_resolution_file native_symbol_resolution_implementation native_symbol_resolution_tool native_symbol_resolution_test].each do |key|
      assert File.file?(File.join(ROOT, documents.fetch(key))), key
    end
  ...
  def test_trial_by_fire_inventory_runs_the_contract_tool
    ...
    assert_includes tools, 'tools/native_symbol_resolution_integration.rb'
