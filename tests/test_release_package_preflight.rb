...
class TestReleasePackagePreflight < Minitest::Test
  ...
  def test_manifest_requirements_are_current_versioned
    ...
    assert_equal 'APPLY_BASIC_SHARP_v0_1_70.sh', spec.fetch('required_installer')
    assert_equal 'v0.1.69', spec.fetch('required_base_version')
    assert_equal 'v0.1.70', spec.fetch('required_target_version')
  end
