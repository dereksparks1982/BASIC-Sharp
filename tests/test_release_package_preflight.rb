...
class TestReleasePackagePreflight < Minitest::Test
  ...
  def test_manifest_requirements_are_current_versioned
    ...
    assert_equal 'APPLY_BASIC_SHARP_v0_1_74.sh', spec.fetch('required_installer')
    assert_equal 'v0.1.73', spec.fetch('required_base_version')
    assert_equal 'v0.1.74', spec.fetch('required_target_version')
  end
  ...
  def test_forbids_unproven_package_handoff
    ...
    assert_includes forbidden, 'removing the DKLab compatibility bridge in v0.1.74'
